...
public class RssReader implements OnFeedLoadListener {
    ...
    private static String ENCODED = "content";
        ...
        if (position != mUrlList.length) {
            ...
            if (mMaterialDialog != null) {
              //------  mMaterialDialog.setContent(source);
                 mMaterialDialog.setContent("Aguarde...");
            }
        } else {
            if (mMaterialDialog != null) {
    ...
    private RssItem getRssItem(Element element) {
        String encoded = "";
        try {
            //encoded = element.select("content:encoded").text();
            //Toast.makeText(mContext, encoded, Toast.LENGTH_SHORT).show();
